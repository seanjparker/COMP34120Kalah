...
public class Protocol {
	...
	public static class MoveTurn {
		...
		/**
		 * The number of the hole that characterises the move which has been made (the
		 * move starts with picking the seeds from the given hole) or -1 if the opponent
		 * has made a swap.
		 */
		public int move;
	}
	...
	/**
	 * Determines the type of a message received from the game engine. This method
	 * does not check whether the message is well-formed.
	 * 
	 * @param msg The message.
	 * @return The message type.
	 * @throws InvalidMessageException if the message type cannot be determined.
	 */
	...
	/**
	 * Interprets a "new_match" message. Should be called if getMessageType(msg)
	 * returns MsgType.START
	 * 
	 * @param msg The message.
	...
	/**
	 * Interprets a "state_change" message. Should be called if getMessageType(msg)
	 * returns MsgType.STATE
	 * 
	 * @param msg   The message.
	 * @param board This is an output parameter. It will store the new state of the
	 *              Kalah board. The board has to have the right dimensions (number
	 *              of holes), otherwise an InvalidMessageException is thrown.
	 * @return information about the move that led to the state change and who's
	 *         turn it is next.
	 * @throws InvalidMessageException if the message is not well-formed.
	 * @see #getMessageType(String)
	 */
	public static MoveTurn interpretStateMsg(String msg, Board board) throws InvalidMessageException {
		...
		/*
		 * if (boardParts.length % 2 != 0) throw new
		 * InvalidMessageException("Malformed board: odd number of entries.");
		 */
		if (2 * (board.getNoOfHoles() + 1) != boardParts.length)
			throw new InvalidMessageException("Board dimensions in message (" + boardParts.length
					+ " entries) are not as expected (" + 2 * (board.getNoOfHoles() + 1) + " entries).");
		try {
